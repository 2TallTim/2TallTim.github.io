...
var BezierRenderer = function(canvas){
    ...
    BezierRenderer.pingPongDemoU = function(){ //Animate the demoU for the deCasteljau parameter
        ...
        x = BezierRenderer.demoU;
        BezierRenderer.demoU = 3*Math.pow(x,2) - 2*Math.pow(x,3);
        // BezierRenderer.demoU = 6*Math.pow(x,5) - 15*Math.pow(x,4) + 10*Math.pow(x,3);
            ...
            while(pl.length>1){
                ...
                for (var i = 0; i < lastPl.length-1; i++) {
                    if(depth>1){ //Draw lines, but not control poly lines
                        ...
                        ctx.fillStyle = "#444";
                        ctx.beginPath();
                        ctx.arc(lastPl[i].x,lastPl[i].y,4,0,Math.PI*2);
                        ctx.fill();

                        ctx.beginPath();
                        ctx.arc(lastPl[i+1].x,lastPl[i+1].y,4,0,Math.PI*2);
                        ctx.fill();
